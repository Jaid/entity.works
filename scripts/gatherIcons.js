...
async function job() {
  ...
  const f = path.resolve(__dirname, "..", "src", "addOns")
  for (const value of Object.keys(deadByDaylight.addOns)) {
    const p = path.join(f, value, "effect.txt")
    await fsp.outputFile(p, "")
  }
